...
if [ X$NJOB == X ]; then
    ...
    # Make output folder
    mkdir -p /scratch_path/$JOBNAME
    mkdir -p /storage_path/$JOBNAME
    mkdir -p $HOME/tests/$JOBNAME/done

fi

#
# Quick termination of job sequence - look for a file called STOP_SEQUENCE
#
